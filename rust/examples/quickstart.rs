...
async fn main() -> kumiho::Result<()> {
    ...
    let project = client.create_project("rust-demo", "Kumiho Rust SDK demo").await?;
    ...
    rev.create_edge(&texture_rev, EdgeType::DEPENDS_ON, None).await?;
    ...
    // Full-text search.
    let hits = client
        .search("hero", "rust-demo", "", false, false, false, 0.0, None, None)
        .await?;
    ...
    if let Ok(mut stream) = client.event_stream("revision.*", "", None, None, false).await {
